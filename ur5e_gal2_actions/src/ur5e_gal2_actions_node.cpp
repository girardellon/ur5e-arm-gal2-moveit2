  ...
  std::chrono::milliseconds timeout = 8000ms)
{
  ...
  while (rclcpp::ok()) {
    ...
    if (std::chrono::steady_clock::now() - start > timeout) {
      RCLCPP_ERROR(node->get_logger(), "Timeout waiting for current robot state (joint_states).");
      return false;
    }
  ...
  const std::vector<double>& joint_values)
{
  if (joint_names.size() != joint_values.size()) {
    RCLCPP_ERROR(node->get_logger(),
                 "joint_names size (%zu) != joint_values size (%zu)",
  ...
  if (!mgi.setJointValueTarget(joint_names, joint_values)) {
    RCLCPP_ERROR(node->get_logger(),
                 "setJointValueTarget(names, values) rejected target (wrong names/size?)");
    return false;
  }
  ...
  if (plan_rc != moveit::core::MoveItErrorCode::SUCCESS) {
    RCLCPP_ERROR(node->get_logger(), "Planning failed (code=%d)", plan_rc.val);
    return false;
  }

  auto exec_rc = mgi.execute(plan);
  if (exec_rc != moveit::core::MoveItErrorCode::SUCCESS) {
    RCLCPP_ERROR(node->get_logger(), "Execution failed (code=%d)", exec_rc.val);
    return false;
  }
...
int main(int argc, char** argv)
{
  ...
  auto node = rclcpp::Node::make_shared("ur5e_gal2_actions");

  // -------- Parameters (arm-only OR arm+gal2) --------
  const bool enable_gripper   = node->declare_parameter<bool>("enable_gripper", false);
  const std::string group     = node->declare_parameter<std::string>("planning_group", "ur_manipulator");
  ...
  // IMPORTANT: For arm+gal2, set these in YAML to include gripper joints too.
  std::vector<std::string> joint_names = node->declare_parameter<std::vector<std::string>>(
    "joint_names",
  ...
  // Default home = “quella del primo file” (arm-only)
  std::vector<double> home = node->declare_parameter<std::vector<double>>(
    "home_joint_values",
    std::vector<double>{ 0.0, -1.57, 0.0, -1.57, 0.0, 0.0 }
  );

  // Optional “grasp” (se vuoi una posa diversa)
  std::vector<double> grasp = node->declare_parameter<std::vector<double>>(
    "grasp_joint_values",
    home
  );
...
#if ENABLE_GRIPPER
  const std::string gripper_port = node->declare_parameter<std::string>("gripper_port", "/dev/contactile_gripper");
#endif

  // -------- Sync robot_description from move_group --------
  if (!importMoveItDescriptionsFromMoveGroup(node)) {
    RCLCPP_ERROR(node->get_logger(), "Failed to import robot_description(_semantic) from /move_group.");
    rclcpp::shutdown();
    return 1;
  }

  // -------- Executor thread for node callbacks --------
  ...
  // -------- Sanity check sizes --------
  if (joint_names.size() != home.size()) {
    RCLCPP_ERROR(node->get_logger(),
                 "Config error: joint_names (%zu) != home_joint_values (%zu). Fix YAML/params.",
                 joint_names.size(), home.size());
    rclcpp::shutdown();
    spinner.join();
    return 1;
  }
  if (joint_names.size() != grasp.size()) {
    RCLCPP_WARN(node->get_logger(),
                "grasp_joint_values size (%zu) != joint_names size (%zu). Using home as grasp.",
                grasp.size(), joint_names.size());
    grasp = home;
  }

  // -------- Main sequence --------
  RCLCPP_INFO(node->get_logger(), "Going to home...");
  if (!goToJointTarget(node, mgi, joint_names, home)) {
  ...
  RCLCPP_INFO(node->get_logger(), "Going to grasp...");
  (void)goToJointTarget(node, mgi, joint_names, grasp);

  rclcpp::sleep_for(500ms);

#if ENABLE_GRIPPER
  if (enable_gripper) {
    (void)setGripperPort(node, gripper_port);
  ...
  // Small shake (cartesian) around EEF Z
  {
    std::vector<geometry_msgs::msg::Pose> waypoints;
  ...
  (void)goToJointTarget(node, mgi, joint_names, home);
